...
export function Table<T>({
  ...
  return (
    <div className={`w-full ${className}`}>
      {/* header */}
      <div className="grid grid-cols-12 px-6 py-3 gap-3 text-[11px] uppercase tracking-wider text-neutral-500 border-b border-neutral-800">
        {columns.map((col) => (
          <div
            key={col.key}
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row) => (
          <div
            key={rowKey(row)}
            className={[
              'grid grid-cols-12 px-6 py-4 items-center gap-3',
              'border-b border-neutral-900/60 hover:bg-neutral-900/30 transition-colors',
              rowClassName?.(row) ?? '',
            ]
              .filter(Boolean)
              .join(' ')}
